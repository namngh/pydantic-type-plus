...
class UInt16(bytes):
    ...
    def validate(cls, v):
        ...
        if isinstance(v, int):
            ...
            v = v.to_bytes(2, 'little')

        return cls(v)

    def __add__(self, other):
        if isinstance(other, UInt16) or isinstance(other, bytes):
            other = int.from_bytes(other, "little")

        return UInt16.validate(int.from_bytes(self, "little") + other)
    ...
    def __invert__(self):
        return UInt16.validate(~int.from_bytes(self, "little"))
    ...
    def __repr__(self):
        return f'UInt16({int.from_bytes(self, "little")})'
